...
/* Add Title */
exports.addSliding = async(req, res) => {
    try {
    ...
    form.parse(req, (err, fields, files) => {
        ...
        s3.upload(params, (err, data) => {
            ...
            let slidingBanner = { userId: user_id, title: title, content: content, img:data.Key };
            Sliding.create(slidingBanner).then(success=>{
...
exports.getSliding = async(req, res) => {
    try {
        ...
        const data = await Sliding.findAll({  limit: limit, offset:offset});
...
exports.updateSliding = async(req, res) => {
    try {
        const {id, status} = req.body;
        const data = await Sliding.update({status: status},{where:{id:id}});
...
/*Delete Title*/
exports.deleteSliding = async(req, res) => {
    try {
        ...
        s3.deleteObjects(deleteParam, function(err, data) {
            ...
            Sliding.destroy({where:{id:id}}).then(result => {
...
/* Update Title Data*/
exports.updateSlidingData = async(req, res) => {
    try {
        let form = new formidable.IncomingForm();
        form.parse(req, (err, fields, files) => {
            if (err) {
            ...
            if (newimage != null) {
                ...
                s3.upload(params, (err, data) => {
                    ...
                    const slug = slugify(title);
                    let slidingBanner = { userId: user_id, title: title, content: content, img:data.Key};
                    Sliding.update(slidingBanner,{where:{id:id}}).then(success=>{
                        return successResponse(req, res, success, 'User Added Succefully');
                    }).catch();
                });
            }
                ...
                let slidingBanner = { userId: user_id, title: title, content: content};
                Sliding.update(slidingBanner,{where:{id:id}}).then(success=>{
                    return successResponse(req, res, success, 'User Added Succefully');
                }).catch();
            }
...
/* Get Sliding by Id*/
exports.getSlidingById = async(req, res)=>{
    try {
        const {id} = req.body;
        const title = await Sliding.findAll({where:{id:id}});
