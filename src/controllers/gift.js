...
exports.addGift = async(req, res) => {
    ...
    form.parse(req, (err, fields, files) => {
        ...
        s3.upload(params, (err, data) => {
            ...
            let giftBanner = { userId: user_id, title: title, content: content, link:link ,img:data.Key };
            Gift.create(giftBanner).then(success=>{
...
exports.getGift = async(req, res) => {
    try {
        ...
        const data = await Gift.findAll({  limit: limit, offset:offset});
...
exports.updateGift = async(req, res) => {
    try {
        ...
        const data = await Gift.update({status: status},{where:{id:id}});
...
exports.deleteGift = async(req, res) => {
    try {
        ...
        s3.deleteObjects(deleteParam, function(err, data) {
            ...
            Gift.destroy({where:{id:id}}).then(result => {
        ...
        form.parse(req, (err, fields, files) => {
            ...
            if (newimage != null) {
                ...
                s3.upload(params, (err, data) => {
                    ...
                    // console.log('AWS UPLOAD RES DATA', data);
                    const slug = slugify(title);
                    let giftBanner = { userId: user_id, title: title, content: content, img:data.Key};
                    Gift.update(giftBanner,{where:{id:id}}).then(success=>{
                        return successResponse(req, res, success, 'User Added Succefully');
                    }).catch();
                });
            }
                ...
                let giftBanner = { userId: user_id, title: title, content: content};
                Gift.update(giftBanner,{where:{id:id}}).then(success=>{
                    return successResponse(req, res, success, 'User Added Succefully');
                }).catch();
            }
...
exports.getGiftById = async(req, res)=>{
    try {
        const {id} = req.body;
        const title = await Gift.findAll({where:{id:id}});
